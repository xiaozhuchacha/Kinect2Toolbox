                                            ...
                                            const BYTE* pBodyIndexBuffer, int nBodyIndexWidth, int nBodyIndexHeight)
{
        ...
        pBodyIndexBuffer && (nBodyIndexWidth == cDepthWidth) && (nBodyIndexHeight == cDepthHeight))
    {
		try {
			...
			if (SUCCEEDED(hr))
			{
				// Load aligned RGB images
				std::string rgb_filename;
				{
					std::stringstream ss;
					ss << "aligned_rgb_" << std::setfill('0') << std::setw(5) << m_nFrameSaverCounter;
					...
					while (it != end) {
						std::string current_filename = it->path().filename().string();
						if (current_filename.substr(0, 17).compare(ss.str()) == 0) {
							rgb_filename = current_filename;
							break;
						}
						it++;
					}
					if (rgb_filename.empty()) {
						std::cout << "cannot find rgb image";
						exit(-1);
					}
				}

				cv::Mat cvColorMapperImage(cDepthHeight, cDepthWidth, CV_8UC3, 3);
				cvColorMapperImage = cv::imread(("./data/" + rgb_filename).c_str());
				cv::imshow("Aligned RGB Image", cvColorMapperImage);
				cv::waitKey(1);
				cv::cvtColor(cvColorMapperImage, cvColorMapperImage, CV_BGR2RGB);
				...
				// Output depth data
				{
					...
					// write as ply file
					{
						...
						depthFilename << "./data/rgbd_pc_rgbd_" << std::setfill('0') << std::setw(5) << m_nFrameSaverCounter << ".ply";
						...
						while (pBuffer < pDepthBufferEnd)
						{
							...
							pcOutputData << singlePoint.X << " " << singlePoint.Y << " " << singlePoint.Z
								<< " " << int(cvColorMapperImage.at<cv::Vec3b>(y, x)[0])
							    << " " << int(cvColorMapperImage.at<cv::Vec3b>(y, x)[1])
								<< " " << int(cvColorMapperImage.at<cv::Vec3b>(y, x)[2]) << "\n";
