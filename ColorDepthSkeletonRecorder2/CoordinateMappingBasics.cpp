		...
		ppBodies != NULL)
	{
		...
		if (SUCCEEDED(hr1) && SUCCEEDED(hr2) && SUCCEEDED(hr3) && SUCCEEDED(hr4))
		{
			// Save Original RGB Images
			{
				...
				const RGBQUAD* pBuffer = pColorBuffer;
				const RGBQUAD* pBufferEnd = pBuffer + (cColorWidth * cColorHeight);
				IplImage* rgbdMapper = cvCreateImage(cvSize(cColorWidth, cColorHeight), IPL_DEPTH_16U, 3);

				for (int pixelCounter = 0; pixelCounter < (cColorWidth*cColorHeight); ++pixelCounter)
				{
					DepthSpacePoint p = m_pDepthCoordinates[pixelCounter];
					
					if (p.X != -std::numeric_limits<float>::infinity() && p.Y != -std::numeric_limits<float>::infinity())
					{
						int colorX = pixelCounter % cColorWidth;
						int colorY = pixelCounter / cColorWidth;
						cvSet2D(rgbdMapper, colorY, colorX, cvScalar(p.Y, p.X, 0));
					}
				}

				std::stringstream convertedDepthImageFilename;
				convertedDepthImageFilename << "./data/depth2rgb_mapper2_" <<
