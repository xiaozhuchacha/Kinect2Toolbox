		...
		ppBodies != NULL)
	{
		...
		if (SUCCEEDED(hr1) && SUCCEEDED(hr2) && SUCCEEDED(hr3) && SUCCEEDED(hr4))
		{
			...
			// Show and Save Depth Images
			{
				...
				IplImage* cvForegroundMask = cvCreateImage(cvSize(cDepthWidth, cDepthHeight), IPL_DEPTH_16U, 1);
				int pixelCounter = 0;
				...
				while (pBuffer < pDepthBufferEnd)
				{
					...
					// foreground mask
					BYTE player = pBodyIndexBuffer[pixelCounter];
					if (player == 0xff) {
						cvSet2D(cvForegroundMask, y, x, cvScalar(*pBuffer >> 3));
					}
					else {
						cvSet2D(cvForegroundMask, y, x, cvScalar(UINT16(player)));
					}
				...
				convertedDepthImageFilename << "./data/depth2rgb_mapper_" <<
					std::setfill('0') << std::setw(5) << m_nFrameSaverCounter << '_' <<
					std::setfill('0') << std::setw(12) << nTime << ".png";
				cvSaveImage(convertedDepthImageFilename.str().c_str(), rgbdMapper);

				std::stringstream foregroundMaskFilename;
				foregroundMaskFilename << "./data/foreground_mask_" <<
					std::setfill('0') << std::setw(5) << m_nFrameSaverCounter << '_' <<
					std::setfill('0') << std::setw(12) << nTime << ".png";
				cvSaveImage(foregroundMaskFilename.str().c_str(), cvForegroundMask);

				cvReleaseImage(&cvDepthImage);
				cvReleaseImage(&rgbdMapper);
				cvReleaseImage(&cvForegroundMask);
			}
